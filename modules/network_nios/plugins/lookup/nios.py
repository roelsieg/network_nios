...
- name: fetch all networkview objects
  ansible.builtin.set_fact:
    networkviews: "{{ lookup('network.nios.nios', 'networkview',
                         provider={'host': 'nios01', 'username': 'admin', 'password': 'password'}) }}"

- name: fetch the default dns view
  ansible.builtin.set_fact:
    dns_views: "{{ lookup('network.nios.nios', 'view', filter={'name': 'default'},
                      provider={'host': 'nios01', 'username': 'admin', 'password': 'password'}) }}"
...
- name: fetch all host records and include extended attributes
  ansible.builtin.set_fact:
    host_records: "{{ lookup('network.nios.nios', 'record:host', return_fields=['extattrs', 'name', 'view', 'comment']}) }}"
...
- name: get a host record
  ansible.builtin.set_fact:
    host: "{{ lookup('network.nios.nios', 'record:host', filter={'name': 'hostname.ansible.com'}) }}"

- name: get the authoritative zone from a non default dns view
  ansible.builtin.set_fact:
    host: "{{ lookup('network.nios.nios', 'zone_auth', filter={'fqdn': 'ansible.com', 'view': 'ansible-dns'}) }}"
"""
...
from ansible_collections.network.nios.plugins.module_utils.net_tools.nios.api import WapiLookup
from ansible_collections.network.nios.plugins.module_utils.net_tools.nios.api import normalize_extattrs, flatten_extattrs
